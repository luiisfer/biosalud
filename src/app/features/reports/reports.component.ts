...
@Component({
  ...
  template: `
    <div>
      ...
      <div class="bg-white p-6 border border-slate-200 h-[600px] flex flex-col mb-8 shadow-sm">
         <div class="flex justify-between items-center mb-4">
           <h3 class="font-bold text-slate-700 uppercase tracking-wide text-xs">
             @switch (activeTab()) {
               @case ('financial') { Tendencia de Ingresos Semanales }
...
export class ReportsComponent implements OnInit {
  ...
  renderMedicalChart(element: any, width: number, height: number) {
    const data = this.db.doctors().map(d => ({ name: d.name, value: d.referralsLastMonth }));
